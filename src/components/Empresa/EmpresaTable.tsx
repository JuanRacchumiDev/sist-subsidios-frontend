...
import { getEmpresasPaginate } from "../../services/empresaService";
import {
  Empresa,
...
import { EmpresaFilters, EmpresaFiltersData } from "./EmpresaFilters";
...
export const EmpresaTable = () => {
  ...
  const [searchFilters, setSearchFilters] = useState<EmpresaFiltersData>({
    search: "",
  });
  ...
  const fetchData = useCallback(
    async (pageToFetch: number, filtersData: EmpresaFiltersData) => {
      setIsLoading(true);
      const filters = {
      ...
      try {
        const response = await getEmpresasPaginate(currentPage, limit, filters);
  ...
  const handleSearchSubmit = (newFilters: EmpresaFiltersData) => {
  ...
  return (
    <div className="w-full space-y-3">
      <div className="bg-white overflow-hidden">
        <EmpresaFilters onSearch={handleSearchSubmit} />
