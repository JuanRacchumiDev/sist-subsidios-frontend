...
import { getCanjesPaginate } from "../../services/canjeService";
import {
  Pagination,
  PaginationContent,
  PaginationEllipsis,
...
export const CanjeTable: React.FC = ({}) => {
  ...
  const [searchFilters, setSearchFilters] = useState<CanjeFiltersData>({
  ...
  const fetchData = useCallback(
    async (pageToFetch: number, filtersData: CanjeFiltersData) => {
      let filters = {
        id_empresa: "",
      ...
      try {
        const response = await getCanjesPaginate(pageToFetch, limit, filters);
  ...
  const handleSearchSubmit = (newFilters: CanjeFiltersData) => {
  ...
  return (
    <div className="w-full space-y-3">
      <div className="bg-white overflow-hidden">
        <CanjeFilters onSearch={handleSearchSubmit} />
      ...
      <div className="flex items-center justify-between px-3 pb-3">
        <div className="text-[11px] text-slate-500 font-medium">
          Mostrando{" "}
          <span className="text-slate-800 font-semibold">{canjes.length}</span>{" "}
