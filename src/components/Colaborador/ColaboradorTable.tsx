...
export const ColaboradorTable: React.FC = ({}) => {
  ...
  const [searchFilters, setSearchFilters] = useState<ColaboradorFiltersData>({
    search: "",
    documento: "",
  });
  ...
  const fetchData = useCallback(
    async (pageToFetch: number, filtersData: ColaboradorFiltersData) => {
      setIsLoading(true);
      ...
      try {
        ...
        if (result && data) {
          setColaboradores(data as Persona[]);
          if (newPagination) {
            setPaginationInfo({
  ...
  const handleSearchSubmit = (newFilters: ColaboradorFiltersData) => {
  ...
  return (
    <div className="w-full space-y-3">
      <div className="bg-white overflow-hidden">
        <ColaboradorFilters onSearch={handleSearchSubmit} />
