...
import { EspecialistaSHTable } from "./EspecialistaSHTable";
import { Plus, GraduationCap, ArrowLeft } from "lucide-react";
import { buttonVariants } from "../ui/button";
import { cn } from "@/lib/utils";

export const EspecialistaSHList = () => {
  const newRoute = `/especialista-sh/nuevo`;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
        <div className="space-y-0.5">
          ...
          <h1 className="text-xl font-bold text-slate-900 tracking-tight">
            Listado de{" "}
            <span className="text-indigo-600">especialistas Sophia Human</span>
          </h1>
          <p className="text-xs text-slate-500">
            Administra, visualiza y gestiona la información de todos los
            especialistas Sophia Human.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            Nuevo especialista Sophia Human
          </Link>
        </div>
      </div>
