...
export const EspecialistaClienteForm = () => {
  ...
  console.log("---- idEspecialistaCliente ----");
  console.log({ id });
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (isEditMode && id) {
          ...
          console.log({ responseEspecialistaCliente });
          ...
          if (result && data) {
            ...
            console.log({ especialistaCliente });
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    const payload: Persona = {
      ...
      telefono,
      nombre_grupo: "GRUPO ESPECIALISTA CLIENTE",
    };
        ...
        <CardHeader className="border-b border-gray-100 p-6 flex flex-row items-center justify-between bg-gray-50/50 rounded-t-xl">
          <div className="space-y-1">
            <CardTitle className="text-2xl font-extrabold text-slate-800 tracking-tight">
              {isEditMode
                ? `Editar especialista cliente`
                : `Nuevo registro de especialista cliente`}
            </CardTitle>
            <CardDescription className="text-slate-500 font-medium">
              {isEditMode
                ? `Actualización de información de especialista cliente`
                : `Complete la información para registrar un especialista cliente`}
            </CardDescription>
          </div>
          <Button
            variant="ghost"
