...
export const TrabajadorSocialRow: React.FC<Props> = ({ trabajadorSocial }) => {
  ...
  const modalMessage = `¿Deseas <strong>${action}</strong> la persona: <strong>${trabajadorSocial.nombre_completo}</strong>?`;

  const handleShowDetail = () => {
    navigate(`/trabajador-social/editar/${trabajadorSocial.id}`);
  };
    ...
    <>
      ...
      <ConfirmDialog
        ...
        icon={
          <AlertTriangle
            className={
              trabajadorSocial.estado ? "text-rose-500" : "text-emerald-500"
            }
          />
        }
      />
